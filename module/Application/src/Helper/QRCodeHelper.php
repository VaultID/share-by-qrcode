...
class QRCodeHelper {
    ...
    function makeImpl($test, $maskPattern) {
        ...
        for ($i = 0; $i < $this->moduleCount; $i++) {
            $this->modules[] = QRCodeHelper::createNullArray($this->moduleCount);
        }
        ...
        $data = QRCodeHelper::createData($this->typeNumber, $this->errorCorrectLevel, $dataArray);
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray) {
        ...
        return QRCodeHelper::createBytes($buffer, $rsBlocks);
    }
    ...
    function createBytes(&$buffer, &$rsBlocks) {
        ...
        $dcdata = QRCodeHelper::createNullArray(count($rsBlocks) );
        $ecdata = QRCodeHelper::createNullArray(count($rsBlocks) );
        ...
        for ($r = 0; $r < $rsBlockCount; $r++) {
            ...
            $dcdata[$r] = QRCodeHelper::createNullArray($dcCount);
            ...
            $ecdata[$r] = QRCodeHelper::createNullArray($rsPoly->getLength() - 1);
        ...
        $data = QRCodeHelper::createNullArray($totalCodeCount);
    ...
    function getGifBytes($size = 2, $margin = 4) {
        $im = $this->createImage($size,$margin);
        ob_start();
        imagegif($im);
